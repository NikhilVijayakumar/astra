...
Astra provides the building blocks:

| Layer | What Astra Provides |
|-------|-------------------|
| **Components** | Stateless UI primitives via Atomic Design (atoms → molecules → organisms → templates) |
...
| **Foundation** | `ThemeProvider`, `LanguageProvider`, `useTheme`, `useLanguage` |
...
- **Localization keys** must come from the ViewModel hook (via `useLanguage`) and be passed as props to components.
