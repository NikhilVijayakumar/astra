...
export const MultiStepProgressIndicator: FC<MultiStepProgressIndicatorProps> = ({
  steps,
  currentStepId,
}) => {
    ...
    <Box sx={{ width: '100%' }}>
      ...
      >
        {steps.map((step, index) => {
          ...
          const status = step.status ?? (isCurrent ? 'in-progress' : 'not-started');
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: spacing.xs }}>
                <Box
                  ...
                  sx={{
                    ...
                    boxShadow: isCurrent ? `0 0 0 3px ${muiTheme.palette.action.hover}` : 'none',
                    flexShrink: 0,
                  }}
                />
                <Typography
                  variant="body2"
                  sx={{
                    color: isCurrent ? muiTheme.palette.text.primary : muiTheme.palette.text.secondary,
