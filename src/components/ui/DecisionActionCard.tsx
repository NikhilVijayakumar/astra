    ...
    >
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: spacing.md }}>
        <Box>
          ...
          <Typography variant="body1" sx={{ color: muiTheme.palette.text.secondary, mt: spacing.xs }}>
            {description}
          </Typography>
        </Box>
        <Typography variant="monoBody" sx={{ color: muiTheme.palette.error.main }}>
          {expiryText}
        </Typography>
      </Box>
      ...
      <Box sx={{ display: 'flex', gap: spacing.sm }}>
        {actions.map((action) => (
          <Button
            ...
            sx={action.variant === 'text' ? { color: muiTheme.palette.text.secondary } : undefined}
          >
            {action.label}
          </Button>
        ))}
